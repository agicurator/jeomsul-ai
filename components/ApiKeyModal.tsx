...
const ApiKeyModal: React.FC<ApiKeyModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleOpenSelector = async () => {
    try {
      if (window.aistudio?.openSelectKey) {
        ...
        // 가이드라인에 따라 선택 트리거 후 즉시 성공으로 가정하여 연결성을 유도합니다.
        setHasKey(true);
        setTestStatus('idle');
        setMessage('시스템에서 API 키를 참조 중입니다. 연결 테스트를 통해 확인해주세요.');
      }
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
      ...
      {/* Modal Card */}
      <div className="relative glass w-full max-w-md rounded-[2.5rem] overflow-hidden shadow-[0_32px_64px_rgba(0,0,0,0.5)] border border-white/10 animate-in zoom-in-95 duration-300">
        <div className="p-10 space-y-8">
            ...
            <div className="p-4 bg-white/5 rounded-2xl border border-white/5 space-y-3">
               <div className="flex items-start gap-2">
                 ...
                 <p className="text-[11px] text-gray-400 leading-relaxed font-medium">
                   입력하신 키는 사용자의 로컬 환경에서 암호화되어 보호되며, 
                   외부로 노출되지 않고 오직 서비스 제공을 위해서만 참조됩니다.
                 </p>
               </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <button 
              ...
              className="w-full bg-white text-brand-dark py-5 rounded-2xl font-black text-lg hover:bg-gray-200 transition-all flex items-center justify-center gap-3 shadow-xl"
            >
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" />
              </svg>
              {hasKey ? 'API 키 변경하기' : 'API 키 연동하기'}
            </button>

            <button 
              onClick={handleRunTest}
              disabled={!hasKey || testStatus === 'loading'}
              className={`w-full py-5 rounded-2xl font-black text-lg transition-all flex items-center justify-center gap-3 border-2 ${
                hasKey 
          ...
          <div className="pt-4 text-center">
            ...
            >
              Go to Google Gemini Billing Documentation
            </a>
          </div>
        </div>
      </div>
